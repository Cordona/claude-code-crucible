...
---
*Procedure Version: 1.0 — the GTD inbox APPEND wrapper, colocated with its owner the `gtd-inbox-writer` agent. Wraps `$HOME/.claude/skills/procedure-inbox-capture/scripts/capture.sh` (portable POSIX sh, shellcheck-clean, self-contained). The entry wire-shape is `crucible/contracts/inbox-entry.schema.json`; triage/list/process/purge belong to the main thread's `flow-inbox` skill; the agent's safety conduct lives in the `gtd-inbox-writer` agent body.*
