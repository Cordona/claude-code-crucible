...
# Colors: honour NO_COLOR (https://no-color.org/) and disable when stdout is
...
resolve_ambiguous() {
	...
	while [ "$ra_retries" -lt "$MAX_RETRIES" ]; do
		printf 'Select [1-%d] (or "c" to cancel): ' "$ra_total"
...
print_welcome() {
	printf '\n'
	printf '===============================================================================\n'
	printf '  %sGitHub CLI Account Manager%s\n' "$C_BLUE" "$C_RESET"
	printf '===============================================================================\n'
	printf '\n'
...
do_switch() {
	...
	while [ "$ds_retries" -lt "$MAX_RETRIES" ]; do
		printf 'Select [1-%d] (or "c" to cancel): ' "$ds_total"
...
main_menu() {
	...
	printf '\n'
	printf '  1) Switch to another logged-in account\n'
	printf '  2) Log in a new account\n'
	printf '  3) Keep the current account\n'
	printf '\n'
