...
**This applies uniformly to production code and test code.** There is no separate, looser standard for tests. A test file that "decorates" every assertion with a docblock is held to the exact same bar as a production module that does the same — `lens-self-documenting-code-reviewer` reviews comments/docstrings everywhere, in production and test files alike; structural quality (production files) is `lens-clean-code-reviewer`'s, and test-specific correctness AND **tests-as-documentation** — whether a test's own structure communicates behavior, distinct from whether its comments do — is `lens-test-quality-reviewer`'s. See `review-boundaries` for the full split.

This skill does NOT contain: structural quality — SRP, DRY/YAGNI, coupling, nesting, dead code, file layout (`standard-clean-code`); the builder's workflow (`build-core`); the lens's own detective method and `category` vocabulary (genuinely `lens-self-documenting-code-reviewer`'s own); the base severity scale, and universal finding-quality/false-positive discipline (`review-core` / `review-report-standards` — the lens only maps its own categories onto that scale); or language-specific idioms beyond comment syntax (the `{tech}` developer/reviewer).
...
Every comment falls in one bucket — only WHY/RATIONALE and PUBLIC-API DOC below are warranted *as documentation*; FUNCTIONAL is not documentation at all and is kept regardless; everything else is REDUNDANT:

| Bucket | Definition | Verdict |
|--------|------------|---------|
| **REDUNDANT** | Restates the code; compensates for a bad name or an overlong unit; a changelog/journal entry (version control already owns this); a mandated comment written only because "every function needs one," not because this one does; a position marker or decorative banner; an attribution comment; commented-out code; an ownerless `TODO` | remove it — the fix is in the *code*, per the Philosophy section above |
| **WHY / RATIONALE** | Explains what code cannot: a workaround + issue link, non-obvious ordering, a genuinely non-obvious algorithm or derivation, a constant from an external spec, a deliberate deviation, a warning of a non-obvious consequence, an amplification of an easily-missed detail, a legal/regulatory notice, an owned `TODO` | keep |
...
- **Extreme single instance (calibrated bar — no recurrence required)** — a comment/docblock **at or above a 5:1 ratio to the code it documents, once the comment block itself exceeds roughly 8-10 lines** (the floor is a second, independent gate, not another way of stating the ratio — a short comment can clear the ratio alone without being a real problem: a 6-line comment over a 1-line field is already 6:1, but at only 6 lines it isn't yet substantial enough on its own to flag without the floor also being cleared), is flagged **on its own, with no recurrence required** — this is the bar a 20-30 line docblock over a single field declaration clears immediately.
...
## Self-Documenting Code consistency

Use the **project's existing comment/docstring conventions** — its own doc-comment sections, its own tolerance for a WHY note's length and placement. Don't impose a foreign house style. But a genuinely redundant comment or an undocumented public contract is a defect regardless of project convention; conformance never launders it.
