---
name: standard-react
description: The single definition of idiomatic, correct, accessible React — the shared rubric the react-developer BUILDS to and the react-reviewer REVIEWS against, composed alongside `standard-typescript` for language-level TypeScript/Zod discipline. Applies whenever React/`.tsx` code (including Next.js and Remix) is written, changed, or reviewed; covers props/component typing, state/hooks/effects, forms, SSR/hydration, render performance, WCAG 2.2 accessibility, output-safety/Server-Action-authorization (§9), and lint/type-check discipline (§10). Defines WHAT good looks like only — not TypeScript language discipline (`standard-typescript`), builder workflow (build-core), the reviewer's correctness-detective method, category vocabulary, audit method, and its own scope-boundary table (genuinely react-reviewer's own), the base severity scale/handoff mechanism (review-core / review-report-standards), or the universal cross-cutting standards (clean-code, self-documenting-code, security, performance, observability).
---
...
This skill defines **WHAT good looks like**. It deliberately does NOT contain: TypeScript language discipline (`standard-typescript`), the builder's workflow (`build-core`); the reviewer's `category` vocabulary, correctness-detective method, accessibility *audit method* (how to verify), and its own scope-boundary table (genuinely `react-reviewer`'s own); the base severity scale, the handoff mechanism, and universal finding-quality/false-positive discipline (`review-core` / `review-report-standards` — `react-reviewer` only maps its own categories onto that scale); or the universal cross-cutting standards (`standard-clean-code`, `standard-self-documenting-code`, `standard-security`, `standard-performance`, `standard-observability`) — with one deliberate, narrow exception: §9 below states the React-specific output-safety and Server Action authorization rules directly, since they're inseparable from React's own rendering/action surface, rather than leaving this pair with no rule on `dangerouslySetInnerHTML`/URL-scheme validation/Server Action auth until a generic security pass happens to cover it.

Assume fluent React and TypeScript, and strict-mode discipline per `standard-typescript`. Assume **React 19**; the React Compiler is opt-in, so read its state from the project's build config and default to **not enabled** when the brief is silent (§7's "Otherwise" branch is the default, not the exception). A Next.js App Router / Remix meta-framework is assumed where one is in use — §6's Server Component rules apply only then; a plain SPA (Vite, react-router) has no RSC boundary to violate. This is **not a tutorial** — it encodes the non-default priorities, idioms, and easy-to-miss traps that separate correct, accessible React from code that merely renders.

## Philosophy

- **Correctness before cleverness.** Deterministic render, honest effects, and exhaustive state handling come first — measuring before optimizing is `standard-performance`'s own model, not restated here.
...
- **List keys must be stable and identity-based** — never the array index when the list can reorder, insert, or delete, or when only a window of the list is mounted (virtualization, where the index rebinds to a different row as the window scrolls), because index keys bind component state to the wrong row. Conversely, a **deliberately changing `key`** is the idiomatic way to remount and reset a subtree when the underlying entity changes.
- **Forms:** React Hook Form + a **Zod schema as the single source of truth**; share that schema client↔server and **re-validate on the server** — server-side validation as a trust boundary is `standard-security`'s own rule, not restated here.
...
- **Correctness first; optimize a *proven* re-render problem, not a guess** — the same measure-before-you-optimize model as the Philosophy section above, applied here to re-renders specifically.
...
- Otherwise: keep **references stable** where they gate memoization (dependency-identity stabilization is §4's rule, not restated here) — a new inline object/function/array identity breaks `memo` and triggers re-render cascades; unstable context values are §2's rule.
- **Virtualize large lists** (>~100 rows) to bound reconciliation and DOM-node cost — key stability is §5's rule, not restated here.
- **Route-level splitting is automatic under Next.js App Router / Remix** — don't hand-roll it there. Use `next/dynamic` / `React.lazy` for a heavy **client** component or below-the-fold tree, and watch for accidental **barrel-file imports** bloating the bundle. Wrap async/lazy trees in **Suspense / error boundaries**.
...
- **Keyboard:** everything operable by keyboard; **visible focus** (never `outline:none` without a replacement); logical tab order; **Esc closes overlays** (modal focus-trap/return is the Dialogs bullet below). Never use a **positive `tabindex` (>0)** (it hijacks tab order) or put `tabindex` on non-interactive elements.
...
- **Focus management:** move focus on route changes (modal open/close focus trap and return is the Dialogs bullet above, not restated here).
...
Every rule below is about the specific places React's safe rendering default is opted out of, or where a request reaches server code directly — the general principle behind why that matters is `standard-security`'s own, not restated here.
...
## 10. Lint & type-check discipline

- **`tsc --noEmit` clean is this gate's baseline** (`standard-typescript` owns `strict: true` and the language rules that make it meaningful; this file owns the gate itself). On top of it, `react-hooks/exhaustive-deps` is treated as an **error**, not a warning — a disabled or downgraded rule is a defect, not a style call (`jsx-a11y/*`'s error-not-warning treatment is §8's rule, not restated here). An unjustified `eslint-disable` carries the same bar as any other unjustified suppression.
