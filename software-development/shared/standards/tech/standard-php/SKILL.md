---
name: standard-php
description: The single rubric for idiomatic, correct PHP — what the php-developer BUILDS to and the php-reviewer REVIEWS against. Applies whenever PHP code (plain, Laravel, Symfony, Doctrine, Eloquent) is written, changed, or reviewed. Covers strict typing/modern idioms, type-juggling and float-comparison traps, null/array-access safety, boundary decoding, exhaustiveness, arithmetic, exception discipline, and ORM/framework correctness. Does NOT define builder workflow (build-core), reviewer mechanics/severity (php-reviewer), generic security (standard-security), or clean-code/performance/testing/observability (their own standards).
---
