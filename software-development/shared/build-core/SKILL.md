...
The orchestrator's brief may narrow what a standing rule demands — *"debug profile only, no `--release`"*, *"don't run the full suite, just this module"*, *"skip the formatter, I'll handle it"*. **Which wins depends on what the gate is FOR, and the three kinds are not negotiable in the same way:**
...
| **Separation of duties** — test authoring, review independence, who is allowed to grade whom | preserving an independent check on your own work | **NEVER.** A brief that collapses two roles into one is void, however mechanical the resulting edit looks. |

> **Why this third row exists.** *"Make the minimal mechanical change so the tests compile"* reads like a verification gate — small, local, "just make it pass" — but it is asking you to author or alter a test, which is a role, not a gate. That framing is exactly the shape this row rejects.
...
**Never invert this.** "The orchestrator told me to" is a reason to skip **running** a test; it is never a reason to write or edit one, commit, push, tag, or confirm an identity on someone's behalf. If a brief asks you to treat a relayed message as the user's authorization, it is asking for the one thing you cannot give.
...
---
*Skill Version: 1.8 — a consumer question found every developer agent bound `standard-testing` despite the absolute ban (this skill's own Constraints) on ever writing or editing a test file — a rubric bound with nothing in it any of them could lawfully act on. Read `standard-testing` end to end: all 10 sections are test-*authoring* guidance (mocking discipline, AAA structure, golden assets, flakiness); none of it is "how to make production code testable," and that guidance was never missing — it already lives here, in Builder Role and Implementation Workflow's "design for testability" step. Removed `standard-testing` from this Overview's bound-standards list and the Pair-with line, and from all 9 developer agents' (the 8 `{tech}-developer`s plus `devops-engineer`) own skill/prose/table bindings (each kept its explicit no-test-authoring Constraint, unchanged) plus the tech-pair generation template, `flow-implementation`'s Direct-implementation step, two residual "universal cross-cutting standards" mentions in `standard-java` and `standard-react`, and the two "developers build to it" mentions in `standard-testing` and `lens-test-quality-reviewer` (now "`tests-developer` builds to it"). Only `tests-developer` and `lens-test-quality-reviewer` bind `standard-testing` now.*
*Skill Version: 1.7 — a lens review of the self-documenting-code extraction found this skill's Overview and step-4 comment-economy self-check still cited `standard-clean-code`'s comment classification and Pair-with line after that content moved wholesale to the new `standard-self-documenting-code` — a dangling citation with no rubric left at the other end. Repointed both, and added the new standard to Overview's bound-standards list and the Pair-with line.*
*Skill Version: 1.6 — a round-4 consistency-lens sweep found this step still said "carry Y into the commit brief" after `flow-git-operations` 1.8 gave the deferred rationale a second possible destination (a PR/MR body, for when the commits it was meant for are already landed) — widened to name both.*
*Skill Version: 1.5 — a consistency-lens finding on v1.4 found the report field was cited as "Key Decisions" with no pointer to its owning skill, unlike this same step's adjacent precedent for the Validation field ("per `build-report-standards`") — reworded to cite the field by its canonical name ("Key decisions") plus owner on first mention, matching that precedent.*
*Skill Version: 1.4 — an execution test of v1.3's comment-economy self-check found it told the developer to compare against "the commit message or PR/MR description" as if that content already existed, when in this framework the developer never authors commits and the message doesn't exist yet at build time — an intent to defer content there had no stated path to actually reach `git-operator` later. Reworded to "intend to leave for" and required the deferred content be named explicitly under Key Decisions, so the primary agent can carry it into the commit brief instead of it silently landing in neither place.*
*Skill Version: 1.3 — a consumer report found this skill had zero guidance on comment volume — testability, ownership, and "not left for review to catch" were all covered, but nothing constrained how much prose a developer writes, so generation was unconstrained and review could only catch what the (also-then-broken) `lens-clean-code-reviewer` was permitted to flag. Added a comment-economy self-check to Implementation Workflow step 4: compare each new comment block's length to the unit it documents before finishing, and disclose an outlier plus its justification under Key Decisions rather than leaving it for review to discover. Pairs with `standard-clean-code` 1.1's new proportionality rule and `lens-clean-code-reviewer`'s new aggregate volume/duplication pass — three sides of one fix, since this skill's "no daylight between build and review" principle (see Overview) means a rubric-only fix would have left generation unconstrained.*
*Skill Version: 1.2 — a lens round on v1.1 found 5 issues, all fixed: the Separation-of-duties row's ~60-word rationale was trimmed to its verdict alone, with the worked example moved to a callout blockquote beneath the table (matching this repo's own convention for "why" asides); the recovery procedure for a developer's own change breaking test compilation was moved out of the Constraints bullet (which now states only the prohibition + a pointer) into Implementation Workflow step 5, where a developer actually is when the gate goes red; the illustrative brief was previously quoted twice with different wording — now quoted once, referenced by pointer the second time; "`tests-developer` REPAIR-mode dispatch" (a term that existed nowhere else in the framework) is now "a `tests-developer` dispatch with repair scope," cited to `flow-testing` §2-3; and "a diff read" was dropped as an alternative to live validation for unlocking that dispatch, since `flow-testing` §0 does not recognize it as a confirmation shape. v1.1 added the row itself — see below.*
*Skill Version: 1.1 — added a third precedence category, "Separation of duties" (§ "When the task brief contradicts a standing gate"), after an incident where a developer edited test files on an explicit "minimal mechanical" brief: the prior two-category table (Verification/Authorization) had no category for who-grades-whom rules, and its own permissive-verification-gate wording ("a reason to skip a *test*") read as license when placed beside it. Reworded that line to "skip **running** a test" and resolved the adjacent open question — who fixes tests broken by a legitimate signature change — via the existing `tests-developer` repair-scope dispatch, so the new hard rule doesn't create a deadlock. The restatement was also added to all 8 `{tech}-developer` agent bodies (and the tech-pair generation template) so it survives a partial skill load.*
