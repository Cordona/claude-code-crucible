...
**A prompt instruction alone is not sufficient enforcement of this rule, and this skill says so rather than implying otherwise.** A real review of this exact class of agent found source material used as writing input — a handoff brief — that was itself full of all three (names, a ticket number, and local paths), which is exactly the kind of source an agent naturally quotes or paraphrases from while drafting. An instruction that says "don't include these" competes, at generation time, against source material that already contains them; nothing about a prompt-level rule reliably wins that competition.
...
---
*Standard Version: 1.6 — a final comprehensive review found the Redaction discipline section's
description of the tech-writer's own `Grep` pattern scan still described it as an unconditional
"deterministic tool-driven scan," matching the exact bare `[A-Z]{2,}-\d+` shape that `doc-lint.sh` v1.1
had already been fixed to stop flagging mechanically (it matches `UTF-8`, `SHA-256`, `RFC-2119`,
`ISO-8601`, and similar ordinary technical vocabulary, not just real ticket IDs) — the fix landed in the
script layer but was never propagated back to this section's description of the agent's own self-scan,
which still implied "find it, remove it" with no judgment step. Reworded to state the ticket-ID half is
deterministic in *finding* matches but requires the tech-writer's judgment to *act* on them, mirroring —
by judgment, since the agent doesn't maintain a copy of the script's allowlist — the same distinction
`doc-lint.sh` applies mechanically. See the matching fix in the tech-writer agent body itself.*
*Standard Version: 1.5 — two fixes from a third, deeper independent review round, both against gaps a
from-scratch compliance audit found in this skill's own excellence checklist. (1) **The Excellence
checklist had zero safeguard against cutting essential content**, despite the skill's own grounding
instruction that it is "very important to not miss essential stuff" — every existing item polices
cutting/minimalism; nothing checked for UNDER-inclusion. Added a new checklist item (and a matching
"Leave out" entry) that requires re-reading the draft against the one-task goal specifically to confirm no
genuinely necessary step, fact, or precondition is missing — a real, checkable counterweight, not a
restatement of the existing minimalism rules. (2) **Added a new minimalism rule (point 6) for a single
volatile fact scattered across multiple mentions WITHIN one document** (a URL, hostname, endpoint,
version, or port repeated more than once in the same doc's body) — distinct from "Say it once" (point 5,
scoped to redundancy ACROSS documents) and from the existing "volatile facts referenced/generated, not
restated" checklist item (about not hand-restating something machine-generated). Every mention past the
first is a second place that fact can go stale independently of the first; state it once, then refer back
or link internally. Updated the matching checklist bullet and "Leave out" catalogue entry to match. No
other content changed.*
*Standard Version: 1.4 — closes the redaction finding this skill's own Redaction discipline section used
to leave open. Updated that section's second bullet to state that the genuinely independent,
orchestrator-run mechanical gate is now real — `procedure-doc-lint`'s `doc-lint.sh`, run by
`flow-documentation` (Step D5) against every tech-writer draft, from outside the authoring agent, before
any fact-check or PR step — rather than continuing to describe it as an open TODO. The gate closes the
pattern-matchable half of redaction (ticket-IDs, local paths) independently of the tech-writer's own
`Grep` pass, plus enforces structure/format (bare code fences, single-item lists) that no script
previously checked at all. It does not extend to person names, and is not a substitute for the
tech-writer's own self-scan — both points stated explicitly so this isn't mistaken for a fuller guarantee
than it is. No other content changed.*
*Standard Version: 1.3 — a re-review of v1.2 found item (3) below was wrong: removing the frontmatter `description:` field's quotes was NOT cosmetic — the description contains a bare `): ` (colon-space) mid-scalar, which YAML parses as a nested mapping once unquoted, so the unquoted frontmatter failed to parse (confirmed via PyYAML: `mapping values are not allowed here`). Of 24 `standard-*` skills, this was the one exception the "22 unquoted" precedent didn't apply to, because this is the one description that happens to contain a colon-space sequence. Re-added the double-quoted form (content unchanged) and confirmed it parses cleanly. No other content changed.*
*Standard Version: 1.2 — three fixes from an independent review fleet's pass over this agent's domain. (1) LLM-readability was previously a strict overlay bounded by the SAME minimalism budget as human-only docs, regardless of audience — but the actual source requirement (a subject-matter expert's own stated need) was that agent-consumability be a PRIMARY driver producing a materially different doc structure for an agent/both audience, not a secondary constraint. Made the LLM-readability section (and its two cross-references, in Philosophy and in the anti-bloat list) explicitly audience-conditional: unchanged strict minimalism for a `human`-only audience, a genuine relaxation — inlining what a link would otherwise carry, when that's what makes a retrieved chunk self-contained — for `agent`/`both`. (2) Updated the Redaction discipline section's description of the tech-writer's self-scan to match that agent's own updated conduct: the ticket-ID and local-path categories now get an actual `Grep` pattern scan (deterministic), while person names remain an LLM judgment pass (not deterministic) — stated honestly as two passes of different reliability, not one blended best-effort review. The genuinely independent, orchestrator-run mechanical gate remains an explicit open TODO, not something the `Grep` half quietly closes. (3) Removed unneeded quoting from the frontmatter `description:` field to match this repo's convention (22 of 24 `standard-*` skills leave an equivalent single-line description unquoted) — cosmetic, no content change.*
*Standard Version: 1.1 — root cause: a subject-matter expert's review of this agent's domain found two gaps a prompt-only rubric can't close by itself. (1) The docs feed multiple AI consumers via chunked retrieval, and nothing here previously required a section to survive being handed to a model without its neighbors — added the LLM-readability section (self-contained sections, per-section acronym expansion, tables for structured facts) and a code-fence discipline (language tag + caption, no bare fences), both additive to the existing anti-bloat rules, not a relaxation of them. (2) A real source document used as writing input for this exact agent's domain was itself full of person names, a ticket number, and internal file paths — proof that a prompt-level "don't include these" instruction is not reliable enforcement — added a Redaction discipline section that states the rule, states plainly that prompt-only enforcement failed in practice, and documents the gap rather than papering over it: an agent self-scan is required now (agent body), a genuinely deterministic mechanical scan does not exist yet and is left as an explicit open TODO for `flow-documentation`. Diátaxis-structure specifics (e.g. "no standalone Tutorial pages"), site-navigation conventions, and file/frontmatter mechanics from that same review were deliberately NOT folded in here — they are one specific docs site's house rules, not general documentation excellence, and conflict with nothing above by being left for a future site-specific skill instead.*
*Standard Version: 1.0 — the shared documentation rubric. Built to by the docs-writer (now tech-writer). Grounded in Diátaxis (the four modes + compass), Carroll's minimalism, and the Google / Microsoft / Write the Docs style canons. It does not define the docs-writer's conduct or report envelope (the agent body).*
