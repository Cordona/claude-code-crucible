...
description: "The single rubric for excellent technical documentation that docs-writer builds to whenever docs are created, updated, or refactored (README, reference, guides, runbooks, architecture, changelog): Diátaxis mode selection, the one-mode/one-reader/one-task gate, anti-bloat minimalism, writing style, and scannability. Defines WHAT good docs look like; it does NOT define the docs-writer's working conduct or report envelope (owned by the docs-writer agent body)."
---
