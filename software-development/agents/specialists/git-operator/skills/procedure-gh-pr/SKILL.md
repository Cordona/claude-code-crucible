...
---
*Procedure Version: 1.2 — the GitHub-PR discovery/creation/editing wrapper. Bound by the git-operator (moved from the project-manager — PR work requires reading the diff, which is development work, not backlog authoring). The account-confirmation gate is the separate `procedure-github-auth`; artifact craft/content is `standard-git-pr`. Sibling to `procedure-gh-issues` (same conventions, different domain). **GitLab backend twin: `procedure-glab-mr`** (same three operations on merge requests; account gate `procedure-gitlab-auth`). Wraps `$HOME/.claude/skills/procedure-gh-pr/scripts/`find-pr.sh, create-pr.sh, update-pr.sh — all portable POSIX, shellcheck-clean, self-contained. **1.2 adds the reciprocal GitLab-twin pointer**, closing a one-directional gap where `procedure-glab-mr` already pointed back here but this file never pointed forward.*
