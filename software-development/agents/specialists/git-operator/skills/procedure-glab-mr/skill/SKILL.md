...
---
*Procedure Version: 1.1 — the GitLab-MR discovery/creation/editing wrapper. Bound by the git-operator, which now handles BOTH GitHub PRs and GitLab MRs. The account-confirmation gate is the separate `procedure-gitlab-auth`; artifact craft/content is `standard-git-pr` (reused as-is — there is no `standard-git-mr`). Sibling to `procedure-gh-pr` (same conventions and safety rule; different CLI, so a different description mechanism and a subgroup-aware path validator). Wraps `$HOME/.claude/skills/procedure-glab-mr/scripts/`find-mr.sh, create-mr.sh, update-mr.sh — all portable POSIX and shellcheck-clean, over two skill-local libraries, `lib/glab-mr-common.sh` and `lib/glab-mr-output.sh`. **1.1 decomposes the three script monoliths onto those libraries** — behavior-identical, verified by the unchanged test suite (328 checks) — and corrects this file's former "self-contained (no external library sourcing)" claim, which the split made false. The output helpers are a separate file on purpose: they parse untrusted glab output and carry the SEC-001/002/003 history, so they change for reasons the rest of the lib does not share.*
