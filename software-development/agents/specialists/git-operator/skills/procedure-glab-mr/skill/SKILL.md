...
description: The procedure the git-operator runs to find, open, and edit GitLab merge requests (MR) via deterministic scripts, never a hand-authored `glab mr` command or a shell-built description. Before any create/update write, the caller must already have explicit user write-consent and have cleared the `procedure-gitlab-auth` account/host gate — this skill does not perform that check itself. It does NOT define MR body/title craft or content (`standard-git-pr`); its GitHub pull-request counterpart is `procedure-gh-pr`.
---
...
`procedure-gitlab-auth`'s gate confirms an **(account, HOST)** pair with the user before any write — but `find-mr.sh` and `update-mr.sh` have no local checkout to anchor that confirmation to. Left unbound, `glab` resolves its target **instance** from ambient state (the cwd's git remotes, an inherited `$GITLAB_HOST`, glab's own config, its `gitlab.com` default). On a machine with **two GitLab instances configured** — exactly the case `procedure-gitlab-auth` exists to disambiguate — the gate could confirm host A while the actual query or edit silently lands on **host B**, whenever the same `--repo` project path resolves on both. A live tracker write is unretractable, and there would be no error to notice.

That is why `find-mr.sh` and `update-mr.sh` take **`--confirmed-host HOST`, and it is REQUIRED** — a usage error (exit `2`) when missing. Required, not optional: an "optional but you really should pass it" middle state leaves exactly the silent-wrong-host path open.
...
### The shared libraries (`lib/glab-mr-common.sh` + `lib/glab-mr-output.sh`)

All three scripts **source two skill-local libraries**, resolved from the script's own `$0` by shell parameter expansion (never `dirname`/`readlink`/`realpath`, none of which may be assumed present):

- **`lib/glab-mr-common.sh`** — the `LC_ALL`/`PROG` pins and `warn`/`error`, `need_arg`, the glab chattiness pins, `is_valid_gitlab_project_path`, `is_valid_confirmed_host`, `is_positive_int`, `split_csv_list`/`accumulate`, the `glab`/`awk`/auth preconditions, `init_tmp_err` (with its load-bearing absolute-path normalization and its `EXIT`/`INT`/`TERM` traps), and `emit_captured_stderr`.
- **`lib/glab-mr-output.sh`** — the helpers that read glab's **untrusted output**: `normalize_mr_rows`, `extract_mr_url_candidates` (including the SEC-002 host-anchored project match), and `count_lines`.

**Why two files and not one:** the output helpers parse bytes this skill does not control — glab's rendered stdout/stderr, which can carry an MR title an outside party chose. They change for reasons the diagnostics and validators never share. **`procedure-gh-pr` has one lib, not two, and that asymmetry is correct**: `gh` answers in JSON via `--jq`, so it has no shape-matching surface at all. Do not force the two skills to match.

**What stays in each script:** its own `usage()`, its own `glab` argv builder (a `set --` inside a function rebinds only that function's positional parameters, so this cannot be centralized even in principle), and its own **policy** for a 0/1/2+ candidate count — `create-mr.sh` exits 1 on ambiguity because its URL is load-bearing, while `update-mr.sh` only warns and empties `PM_MR_URL` because its URL is a documented courtesy.

**Neither library is ever sourced across skills.** Nothing here sources `procedure-gh-pr`, even though `warn`/`error`/`need_arg`/`is_positive_int`/`split_csv_list` are byte-identical to that skill's copies: the two skills deploy to the hub **independently**, so a cross-skill source would resolve to a path that may simply not be there, and it would fail at *runtime*, invisibly, on the first real MR operation. Those helpers are **duplicated on purpose**; when you change one, change both.
