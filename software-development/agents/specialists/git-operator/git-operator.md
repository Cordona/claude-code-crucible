...
2. **Plan the branch** (if one is needed) — decide the `type/ticket-desc` name per `standard-git-branch`. **The base branch is an input the delegation gives you, never yours to default** (`flow-git-operations`' Branch Path) — the orchestrator asks the human if it wasn't supplied. Include the name + base in the plan for the orchestrator to create via `create-branch.sh`. You do not create it.
