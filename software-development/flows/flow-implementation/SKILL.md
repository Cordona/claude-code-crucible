...
---
*Procedure Version: 1.8 — a consumer question found §6's DIRECT-mode standards list included `standard-testing`, matching what every developer agent bound at the time — but that skill is a pure test-authoring rubric, and DIRECT mode (like every developer) is absolutely banned from authoring tests. Its actual testability guidance was never missing: `build-core`'s own Implementation Workflow already covers it. Removed `standard-testing` from this list (v1.8 pairs with the same fix across `build-core`, `standard-testing`, all 9 developer agents, and the tech-pair template).*
*Procedure Version: 1.7 — a lens review of the self-documenting-code extraction found §6's DIRECT-mode standards enumeration was the one bound-standards list this repo's registries this round missed — every `{tech}-developer` and `build-core`'s own Overview were repointed, but this list wasn't, so DIRECT-mode framework-prose implementation built against zero naming/comment rubric. Added `standard-self-documenting-code`.*
