...
*Procedure Version: 6.3 — a lens review of the self-documenting-code extraction found §2b's seat-routing table still routed "clean-code / SOLID / naming" wholesale to `lens-clean-code`, which now hard-disclaims naming and comments in every file — a PR/MR finding about a name or a redundant comment had no seat that could legitimately claim it. Narrowed the row to "clean-code / SOLID / structure" and added a sibling row routing comments/docstrings/naming to `lens-self-documenting-code`.*
