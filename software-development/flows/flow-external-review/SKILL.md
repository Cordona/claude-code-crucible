...
### 2b. Seat routing — pick the advocate/fix-reviewer by what the finding IS
...
| `sha-delta.txt` | 0 | the commits landed between the reviewed SHA and HEAD — the ALREADY_RESOLVED input |
...
- Record HEAD sha (**run `git` from the repo dir, not the run dir**); record the **reviewed-SHA→HEAD delta** (the commits landed between the reviewed SHA and HEAD) to `sha-delta.txt` — the ALREADY_RESOLVED input.
...
| **`Medium` / `Warning`** | **`HIGH`** | **a deliberate escalation, not a translation.** An external `Medium` is not a consequence judgment — it is a bot's heuristic, and bots under-label real defects. Escalating buys a panel, and the panel is how we find out. (A dual-pass *Medium* data-integrity finding therefore triggers a panel.) |
| `Low` / `Info` | `MEDIUM` | |
...
```
A fix that breaks tests or exceeds scope re-classifies the finding (→ TRACK_FOLLOWUP/BLOCKED) — the fix phase is a safety net beyond adjudication. No commit inside this sub-procedure.
...
**The entire `glab mr note` subcommand family (`create`/`list`/`update`/`delete`/`resolve`/`reopen`) is marked EXPERIMENTAL by glab itself** — its own `--help` text says "This feature is an experiment and is not ready for production use. It might be unstable or removed at any time." This is an accepted, disclosed risk, the same posture this framework already takes toward other known limitations (e.g. `procedure-git-identity`'s disclosed gaps) — not a reason to avoid the feature, since there is no non-experimental alternative for MR discussion management in `glab` today. `--unique`'s no-duplicate behavior, `--resolvable=false`'s never-blocks-merge behavior, and `--reply`'s stale/resolved tolerance (below) are all EXPERIMENTAL-flagged, verified against glab 1.112.0, and could change silently on a glab upgrade. **Re-verify these three specific behaviors live before trusting them again after any `glab` version bump past 1.112.0** — this is a correctness/idempotency risk, not an injection risk (the shell construction's safety doesn't depend on glab's version).

**Divergence from GitHub:** unlike GitHub's reply API, `glab mr note create --reply <discussion-id>` does **NOT** reject a reply once the target discussion is stale or resolved:
