...
One `### <id>` block per finding, in the order they appear in `findings[]`. Render both status axes explicitly, side by side — that's why they're two separate fields rather than one conflated enum (see `tracked-status.schema.json`'s own description for the REGRESSED-and-IN_PROGRESS case this exists to represent).
