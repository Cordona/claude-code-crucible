---
name: flow-spec
description: The orchestrator's procedure for drafting and approving a spec BEFORE implementation starts. Bind for cross-repo/multi-tech-pair work, a genuinely forked single-repo interface design, or an explicit spec-first request. The draft must be approved BEFORE any parallel `flow-implementation` dispatch — 2+ parallel pairs without a governing spec require an explicit human ask. Does NOT run the `flow-decision` panel itself (only proposes one), build against the spec (`flow-implementation`), or review it (`flow-review`).
---
