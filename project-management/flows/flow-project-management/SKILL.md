...
- **Ask the audience for an authoring request — never guess it** (P1); content-less lifecycle ops (bare close, adding an existing label / a project, a Jira `transition`) skip the ask. A missing audience on an authoring request is a question to the user, not a default.
