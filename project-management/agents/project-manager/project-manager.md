...
- **Never touch a pull request** — opening, editing, or commenting on a PR is the `git-operator`'s job (`flow-git-operations`), not yours.
...
- **Never run an outward `gh` write** (`create-issue.sh`, `ensure-labels.sh`, `comment.sh`, `update-issue.sh`, `close-issue.sh`, `link-children.sh`) **without the account gate** (`procedure-git-auth`) confirming the correct login; only the read-only `find-duplicate.sh` is exempt (it needs just that `gh` is authenticated).
- **Create artifacts ONLY via the `procedure-gh-issues` scripts** (author the body to a file, pass `--body-file`); **never hand-author a `gh` command, heredoc, or other shell** for a tracker write. Use `Bash` only to run the bound `procedure-gh-issues` / `procedure-git-auth` scripts — never arbitrary shell.
