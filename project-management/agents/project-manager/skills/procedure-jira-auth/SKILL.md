...
- **`--copy`** — prints the path of a fresh `600` `mktemp` **throwaway copy** (useful for isolation from a concurrent `jira-login.sh` overwrite). The **caller owns cleanup** — `rm -f` it as soon as the engine is done (ideally under the caller's own exit trap); it lives in `$TMPDIR`, not the `700` credential dir. **Do NOT combine `--copy` with per-call fresh resolution** — a fresh copy on every one of many `jira.sh` calls in a turn mints a new token-bearing file each time, multiplying the number of files that need cleanup and the window in which a missed `rm -f` leaves one behind. Prefer the default form for the per-call resolution this skill requires; reach for `--copy` only for a single, isolated read where that specific concurrency concern applies, and clean it up immediately after.
