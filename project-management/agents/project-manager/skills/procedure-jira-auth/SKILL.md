...
This is a **procedure, not a rubric**: run the scripts in order, present, confirm, hand off the credential. It is the Jira analog of `procedure-git-auth` (that gate confirms *which GitHub login acts*; this one confirms *which Jira site + account acts*, and additionally produces the credential handoff the engine consumes).
