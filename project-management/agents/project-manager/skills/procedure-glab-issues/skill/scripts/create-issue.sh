...
for _pm_lib in pm-diag.sh pm-glab-env.sh pm-validate.sh pm-glab-preconditions.sh pm-glab-url.sh pm-glab-labels.sh pm-lists.sh; do
...
# so the whole job is TWO shared helpers in lib/pm-lists.sh — split_csv_list
# (tokenize) and csv_accumulate (append, VALUE-RETURNING) — rather than two
...
# chosen by a name argument: `LABELS=$(csv_accumulate "$LABELS" "$2")` keeps the
...
require_awk "page the label lookup and read the issue URL back"

require_glab_auth

# ---------------------------------------------------------------------------
# Temp file (mktemp, cleaned up on exit — see init_tmp_err in lib/pm-diag.sh) —
# used only for glab's OWN stderr. NEVER used for the issue body, which travels
# as one argv value.
# ---------------------------------------------------------------------------
init_tmp_err pm-glab-create-issue
...
if [ -n "$OPT_MILESTONE" ]; then
	...
	# legitimately contain a double quote (see pm_strip_jq_quotes's own header
	# in lib/pm-diag.sh).
	MILESTONE_TITLES=$(pm_strip_jq_quotes "$MILESTONE_RAW")
...
ISSUE_URL_CANDIDATES=$(extract_issue_url_candidates \
	"$(printf '%s\n%s\n' "$CREATE_OUT" "$(cat "$TMP_ERR")")" "$OPT_REPO" "$OPT_CONFIRMED_HOST")
