...
Each script keeps its own `usage()`, its argument parsing and error strings, and — necessarily — its own `gh` argv builder: a `set --` inside a function rebinds only that function's positional parameters, so building a command's argv cannot be centralized even in principle. The `TMP_ERR` temp file and its trap are not per-script: `init_tmp_err PREFIX` owns them, and `link-children.sh`, which mktemps three more files, re-arms its own trap with the full list after each.
...
- **All four list flags are repeatable AND comma-split**, trimming each token and dropping empties — the same rule `create-issue.sh` follows. `--milestone` is not a list flag.
