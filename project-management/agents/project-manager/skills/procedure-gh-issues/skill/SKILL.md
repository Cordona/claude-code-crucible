...
description: "Procedure the project-manager binds to create, query, comment on, edit, and close GitHub issues, ensure/create repo labels, and link child issues under an epic — via deterministic wrapper scripts, never hand-authored gh shell or string-built bodies (command-injection safety). Requires the project-manager's own creation-consent gate plus the procedure-github-auth account gate cleared before any write; only find-duplicate.sh is read-only/ungated. Does not define artifact craft/content (standard-backlog-artifacts) or the account gate itself (procedure-github-auth). GitLab counterpart: procedure-glab-issues."
---
...
| File | Holds |
|------|-------|
| `lib/pm-diag.sh` | the `LC_ALL`/`PROG` pins, `warn`/`error`/`need_arg`, and `init_tmp_err PREFIX`/`emit_captured_stderr` |
| `lib/pm-validate.sh` | pure predicates: `is_positive_int`, `is_valid_repo_slug`, `is_valid_hex_color` |
| `lib/pm-gh-preconditions.sh` | `require_gh_cli`, `require_awk REASON`, `require_gh_auth` — three SEPARATE functions, because two commands interleave the awk check BETWEEN the gh-present and gh-authenticated checks |
| `lib/pm-lists.sh` | `split_csv_list`, `csv_accumulate` (comma-splits — the path EVERY caller-facing list flag takes), `append_line` (does NOT — internal, already-validated accumulators only) |

Each script keeps its own `usage()`, its argument parsing and error strings, and — necessarily — its own `gh` argv builder: a `set --` inside a function rebinds only that function's positional parameters, so building a command's argv cannot be centralized even in principle. The `TMP_ERR` temp file and its trap are NOT per-script any more: `init_tmp_err PREFIX` owns them, and `link-children.sh`, which mktemps three more files, re-arms its own trap with the full list after each.

**This is the ONE library, and its scope is exactly this skill.** Nothing here sources `procedure-glab-issues`, even though `pm-diag.sh` and `pm-lists.sh` are near-identical to that skill's copies. The two skills deploy to the hub **independently**, so a cross-skill source would resolve to a path that may simply not be there — and it would fail at *runtime*, invisibly past a green test suite, for anyone who installs one skill without the other. Those helpers are **duplicated on purpose**; when you change one, change both. `is_valid_repo_slug` is a different case again — it is deliberately **not** the same validator as `procedure-glab-issues`' `is_valid_gitlab_project_path` (this one hard-rejects a second `/`; that one must accept arbitrary subgroup depth), so the two must **never** be unified behind one parameterized helper. `is_valid_hex_color` likewise differs: this one rejects a leading `#`, the GitLab one accepts it.
...
- **Every list flag — `--label`, `--assignee`, `--project` — is repeatable AND comma-splits, identically.** Each occurrence is split on commas, each token trimmed, empty tokens dropped, and the result accumulated across occurrences: `--assignee alice,bob` and `--assignee alice --assignee bob` are the same request. This matches `update-issue.sh`'s `--add-label`/`--remove-label`/`--add-assignee`/`--remove-assignee`, `ensure-labels.sh`'s `--label`, and the real `gh` CLI's own repeatable flags. The cost is that a value *containing* a comma cannot be expressed through these three — the same limitation `gh` itself has. **`--milestone` is deliberately NOT a list flag** (a single title, passed verbatim, commas and all).
- **`--project NAME` has NO pre-check** — project lookup is GraphQL (projects-v2), out of scope for this script. An unknown project surfaces as a plain `gh issue create` failure, same as any other gh error.
...
- **All four list flags are repeatable AND comma-split**, trimming each token and dropping empties — the same rule `create-issue.sh` now follows. `--milestone` is not a list flag.
...
---
*Procedure Version: 2.3 — the GitHub-issue creation/duplicate-check/linking/labeling/commenting/editing/closing wrapper. Bound by the project-manager. The account-confirmation gate is the separate `procedure-github-auth`; artifact craft/content is `standard-backlog-artifacts`. **GitLab backend twin: `procedure-glab-issues`** (same seven scripts on GitLab issues; account gate `procedure-gitlab-auth`). Wraps `$HOME/.claude/skills/procedure-gh-issues/scripts/`create-issue.sh, find-duplicate.sh, link-children.sh, ensure-labels.sh, comment.sh, update-issue.sh, close-issue.sh — all portable POSIX and shellcheck-clean, over this skill's own `lib/`. **2.1 adds the reciprocal GitLab-twin pointer**, closing a one-directional gap where `procedure-glab-issues` already pointed back here but this file never pointed forward. **2.2 decomposes the seven script monoliths onto that skill-local `lib/`** (`pm-diag`, `pm-validate`, `pm-gh-preconditions`, `pm-lists`) and moves `SKILL.md`+`scripts/` under `skill/` — behavior-identical, proven by running the ORIGINAL pre-refactor scripts and the refactored ones through the same harness and diffing the full transcripts byte-for-byte, plus new comma-list / non-comma-splitting-`--assignee` / predicate-truth-table coverage — and corrects this file's former "self-contained (no external library sourcing)" claim, which the split made false. **2.3 is a deliberate BEHAVIOR CHANGE, the one 2.2 explicitly deferred: `create-issue.sh`'s `--assignee` and `--project` now comma-split** (via the same `csv_accumulate` every other list flag already used), ending the asymmetry where the identical `alice,bob` string meant one assignee on create and two on update. Splitting is the more capable direction and matches both this skill's existing convention and the real `gh` CLI; a caller passing a single value is unaffected, and repeats still accumulate. `--milestone` remains a non-list scalar, and `link-children.sh --child` still rejects a comma-list as a usage error.*
