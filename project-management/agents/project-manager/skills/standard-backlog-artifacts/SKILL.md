...
---
*Standard Version: 1.3 — scoped the audience-matrix's opening "required input" claim to an artifact-AUTHORING dispatch — a lens review of a `flow-project-management` fix found this was the last of 4 PROSE sites in the framework still stating the requirement unconditionally, after the other 3 (in `project-manager.md`) were qualified to admit the content-less-lifecycle-op carve-out. (A follow-up round found and fixed a 5th, non-prose site — `audience-register.schema.json`'s own `description` field, cited by this file and `project-manager.md`'s audience contract.)*
*Standard Version: 1.2 — the shared backlog-artifact rubric. Built to by the project-manager. Grounded in INVEST, vertical slicing, Given/When/Then acceptance criteria, and Diátaxis-style audience calibration. Tracker-agnostic — the creation mechanics live in the project-manager's `procedure-gh-issues` (GitHub), `procedure-glab-issues` (GitLab), and `procedure-jira` (Jira) scripts, never in this rubric. **Pull/merge-request bodies moved to `standard-git-pr`** (git-operator's domain) along with the rest of that lifecycle. It does not define the project-manager's conduct, gates, or report envelope (the agent body). **1.2 updates the tracker list** to reflect GitLab shipping alongside GitHub and Jira (both already implemented, not "later").*
