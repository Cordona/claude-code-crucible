...
- **None of the four auto-fires from repository state anymore.** That was the previous design, and it had a real failure mode: the full lens battery running automatically on every build, at any size, including a misjudged cross-repo effort that cost hours and millions of tokens polishing and testing an implementation that turned out to be wrong. The fix isn't "review less" — it's "review deliberately, and only what was asked for."
- **The one thing that still checks repository state is `flow-git-operations`'s commit gate** — before any commit, it asks whether the diff has been through at least `flow-implementation`'s tech-pair loop, and refuses to assume either answer if unsure. That's the entire surviving safety net for "don't ship something nobody looked at" — cheap, because it only ever asks a question, never re-runs a swarm.
