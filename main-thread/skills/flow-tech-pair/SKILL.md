...
Grep `roles/developers/agents/*.md` and `roles/reviewers/agents/tech/*.md` (names and descriptions) for anything that already plausibly covers the requested language/ecosystem — e.g. for a "TypeScript" request, grep for `typescript\|react\|\.tsx` against those files, since `react-developer` already covers TS+React.
...
> - **Files to be created:**
>   - `roles/common/skills/standards/tech/standard-{tech}/SKILL.md`
>   - `roles/developers/agents/{tech}-developer.md`
>   - `roles/reviewers/agents/tech/{tech}-reviewer.md`
